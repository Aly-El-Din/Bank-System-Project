...
void ClientManager::updatePassword(Person* p)//me7taga tetrage3
{
	FileManager f;
	Employee e;
...
double ClientManager::enterWithdraw(Client* ccc) {
...
void ClientManager::withdraw(Client* client)
{
	client->withdraw(enterWithdraw(client));
	cout << "YOUR NEW BALANCE IS =>> " << client->getBalance() << endl;
	system("pause");
}
double ClientManager::depositting(Client* client)
{
	double amount; string amountS;
	system("cls");
	Log::log("DEPOSIT");
...
void ClientManager::enterClientId(Client* client) {
	int id; string idS;
...
double ClientManager::enterTransfer(Client* client1, Client* client2) {
	double amount; string amountS;
	system("cls");
	Employee* employee = new Employee;
	//Client* client2 = new Client;
...
bool ClientManager::clientOptions(Client* ccc)
{
